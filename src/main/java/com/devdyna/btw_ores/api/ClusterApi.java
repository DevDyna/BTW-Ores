...
public class ClusterApi {
    ...
    public static int getIndex(BlockState state, BlockPos pos, Level level) {
        ...
        if (LevelUtil.isBiome(level, pos, zTags.BIOME_NETHER) && state.is(ItemsBlocks.NETHER_CLUSTER_BLOCK.get())) {
            return 2;

        }

        if (LevelUtil.isBiome(level, pos, zTags.BIOME_END) && state.is(ItemsBlocks.END_CLUSTER_BLOCK.get())) {
            return 3;
    ...
    public static void dropItems(BlockState state, ItemStack pickaxe, Level level, BlockPos pos, Player player) {
        ...
        for (int i = 0; i < LevelUtil.getRandomValue(pickaxe
                .getEnchantmentLevel(Enchantments.BLOCK_FORTUNE), level); i++) {
